...
async function startScraping(scrapeProducts, scrapeWithDetails) {
  ...
  axios
    ...
    .then((response) => {
      ...
      async function getProductsByCatUrl(url, pageNumber = 1) {
                ...
                await axios
                  ...
                  .then((response) => {
                    ...
                    Product.createIfNotExist({
                      ...
                      code: $(
                        ".product-image-summary .sku_wrapper .sku"
                      ).text(),
                      export_code: $(
                        ".product-image-summary .barcode-varedati"
                      ).text(),
                      url: $(product)
                        .find(".product-element-top > a")
                        .attr("href"),
                    });
                  })
                  .catch((error) => {
        ...
        if (pageNumber < totalPages) {
          await delay(defaultDelay);
          getProductsByCatUrl(url, pageNumber + 1);
        }
      }
      ...
      function getSubMenuItems(menuItem, parentId = null) {
        ...
        if (subMenuItems?.length) {
          ...
          getProductsByCatUrl($(menuItem).find("> a").attr("href"), 1);
        }
      }
